...
export default function PatientForm({
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const form = e.currentTarget; // ✅ sauvegardé AVANT le await
    ...
    if (res.ok) {
      form.reset(); // ✅ utilise la référence sauvegardée, pas e.currentTarget
      onSuccess();
    }
    ...
    <form onSubmit={handleSubmit} className="p-6 bg-white rounded-lg shadow-md space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <input name="dateNaissance" type="date" required
          className="p-2 border rounded-lg w-full" />
        <select name="sexe" required className="p-2 border rounded-lg w-full">
        ...
        <input name="telephone" placeholder="Téléphone (optionnel)"
          className="p-2 border rounded-lg w-full" />
        <select name="region" required className="p-2 border rounded-lg w-full">
